// Create a Recharge Server payment session for an SMS package purchase.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: pkg, error: pErr } = await admin
      .from("sms_packages")
      .select("id, name_en, sms_count, price_bdt, is_active")
      .eq("id", packageId)
      .maybeSingle();
    if (pErr || !pkg || !pkg.is_active) return json({ error: "Invalid package" }, 400);

    const finalPrice = Number(pkg.price_bdt);

    const { data: tx, error: tErr } = await admin
      .from("payment_transactions")
      .insert({
        user_id: user.id,
        sms_package_id: pkg.id,
        shop_id: shopId,
        kind: "sms_package",
    ...
    const successUrl = `${origin}/app/sms/callback?status=success&local_id=${tx.id}`;
    const cancelUrl = `${origin}/app/sms/callback?status=cancel&local_id=${tx.id}`;
    ...
    const rsRes = await fetch("https://payment.rechargeserver.com/api/payment/create", {
      ...
      body: JSON.stringify({
        ...
        metadata: { phone: phoneMeta, local_id: tx.id, user_id: user.id, kind: "sms_package", package_id: pkg.id, shop_id: shopId },
