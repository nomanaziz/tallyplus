-- Fix consumer signup: avoid inserting '' into unique phone column.
-- Also clean any stale '' rows (currently none, but safe).
UPDATE public.consumer_profiles SET phone = NULL WHERE phone = '';
...
begin
  if v_account_type = 'consumer' then
    insert into public.consumer_profiles (id, name, phone, country_code)
      values (new.id, v_full_name, nullif(coalesce(new.phone, ''), ''), v_country)
