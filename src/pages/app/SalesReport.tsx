...
import { salesReportQuery, rangeToIso } from "@/lib/queries";
...
function Page() {
  ...
  const { data, isFetching, refetch } = useQuery(salesReportQuery(current?.id ?? null, iso));
  const rows: any[] = (data ?? []) as any[];

  const totals = useMemo(() => {
    let total = 0, paid = 0, due = 0, count = rows.length;
    for (const r of rows) { total += Number(r.total ?? 0); paid += Number(r.paid ?? 0); due += Number(r.due ?? 0); }
    return { total, paid, due, count };
  }, [rows]);
  ...
  return (
    <ReportShell
      breadcrumb="বিক্রির রিপোর্ট"
      titleBn="বিক্রির রিপোর্ট"
